...
int broadcast_message(struct users *users, uint32_t sender, uint32_t len,
                      const char *msg) {
    for (uint32_t i = 0; i < users->len; i++) {
        send_response(users->pfds[i + 1].fd, R_MSG, users->name_lens[sender],
                      users->names[sender], len, msg);
    }
    return NONE;
}
